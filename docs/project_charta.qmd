...
The Carbon Gatekeeper Dashboard is an interactive tool designed to manage and reduce CO₂ emissions from business travel by linking travel lists directly to carbon budgets. Moving beyond retrospective reporting, the dashboard provides real-time, data-driven guidance by analyzing planned itineraries. The project is designed to answer three core questions:

> *"What is the current CO₂ budget status for my business unit?"*
>
> *"Which specific trips in my provided list have viable, lower-carbon alternatives?"*
>
> *"Based on our remaining budget, how much CO₂ can we save by switching to these recommended transport modes??"*

A central component is the **Departmental Budget Monitor**, which uses a gauge system to compare accumulated emissions with annual CO₂ budgets, classifying units as Low, Medium, or High. The dashboard also includes a **Potential Analysis** feature that highlights routes where sustainable alternatives exist. When a user inputs a list of planned trips, the tool automatically scans the entries to highlight specific routes where sustainable alternatives—such as rail, bus, or car equivalents—exist. By comparing these options, the tool quantifies the "avoidable CO₂" for those specific trips to make reduction opportunities immediately actionable. Additionally, an **interactive trip evaluation tool** allows users to filter specific routes and compare travel options based on emissions, cost, and time, providing a recommendation tailored to the unit's current budget status.

The project scope involves developing a functional dashboard prototype that integrates these features into a single interface. This includes implementing CO₂ calculation logic, aggregating emissions data by business unit, and establishing a rule-based system that flags alternative routes within a provided dataset. The primary deliverable is an interactive tool that supports transparent monitoring and informed decision-making. While limited to business travel using existing data and excluding external booking system integrations, the project offers significant benefits in operationalizing sustainability targets. The final product will be published as a web-based prototype via Streamlit and GitHub Pages, ensuring stakeholders can access the tool directly through a browser without additional software.
---
...
- **Impact Comparison** — *"Which specific trips in my provided list have viable, lower-carbon alternatives?"* (quantifying the "avoidable CO₂" for routes where `train_alternative_available` is TRUE)
- **Actionable Guidance** — *"Based on our remaining budget, how much CO₂ can we save by switching to these recommended transport modes?"* Providing tailored recommendations like "MUST TAKE TRAIN" or "TRAVEL RESTRICTED"

### Success Criteria

**Qualitative Objectives**

- **Actionability** — The tool automatically identifies "quick wins" by scanning user-provided trip lists and highlighting specific routes where low-carbon alternatives are viable.
...
- **Feature Integration** — The prototype must successfully integrate three core components: the Budget Monitor (Gauge), the Trip List Comparison (Original vs. Alternative), and the Recommendation Logic into a single interface.
- **Identification Logic** — The tool must successfully match at least 80% of eligible routes (e.g., short-haul flights) in a user-provided list to their corresponding rail or bus alternatives.
- **Comparative Visibility** — For every trip identified as having an alternative, the dashboard must display the delta (difference) in CO₂, cost, and time compared to the original entry.
...
- **Smart Trip List Table** — A dynamic list view of the user’s trips that uses color-coding or icons to flag specific rows where a sustainable alternative has been identified
- **Comparative Map View** — When a flagged trip is selected from the list, the map geographically illustrates the flight path versus the rail/bus alternative to visualize the distance and route difference.
- **Highlight metrics** — Show total CO₂ used, remaining budget, and avoidable CO₂ in a clear, visible format

### Interactivity
Users can upload or select a specific trip list for their department. The dashboard updates dynamically, allowing users to click on trips with identified alternatives to instantly see the map and comparison panels change. Interactive filters for Business Units adjust the budget gauge in real-time, while linked views ensure that choosing a sustainable alternative for a specific trip immediately reflects the projected savings in the department's remaining carbon budget.
